...
import { UploadedImage, TryOnResult, UserStats, ClothingItem } from './types';
  ...
  const handleGenerate = async () => {
    ...
    for (const combo of combinations) {
      ...
      try {
        ...
        setResults(prev => prev.map(r => r.id === resultId ? { ...r, loading: false, error: "Failed to generate look." } : r));
      }
    }
                        ...
                        <div className="space-y-3 mt-2">
                           <div className="flex flex-col gap-1.5">
                              <label className="text-xs font-semibold text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                                <Calendar size={12} /> Occasion (Optional)
                              </label>
                              <input
                                type="text"
                                value={stats.occasion}
                                onChange={(e) => setStats({...stats, occasion: e.target.value})}
                                placeholder="e.g. Wedding Guest, Job Interview, Date Night"
